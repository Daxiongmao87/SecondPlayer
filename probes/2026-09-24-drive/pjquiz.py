"""PlayJev factorized quiz: 3-opt lateral + 2-opt drop on SNES pockets."""
import os, sys, time
sys.path.insert(0, "/tmp/playjev-pub")
os.environ.setdefault("HF_HUB_OFFLINE", "0")
from PIL import Image, ImageDraw
from playjev.model import PlayJevModel, SYSTEM_PROMPT, DEFAULT_INSTRUCTIONS
...
print("SYSTEM_PROMPT:", repr(SYSTEM_PROMPT), flush=True)
print("INSTRUCTIONS:", repr(DEFAULT_INSTRUCTIONS), flush=True)
...
OPTS_H = [{"name": n, "description": n} for n in ("left", "right", "none")]
OPTS_D = [{"name": n, "description": n} for n in ("drop", "none")]

def load(dev):
    m = PlayJevModel("OmniJev/PlayJev-0.8B", device=dev, dtype="bfloat16")
    m.load()
    return m
...
try:
    ...
    print(f"xpu load failed ({type(e).__name__}), falling back to cpu", flush=True)
    model = load("cpu")
    dev = "cpu"
print(f"device={dev} load_seconds={model.load_seconds:.0f}", flush=True)
...
for fps in (1, 2):
    ...
    for name, ((prv, cur), exp_h, exp_d) in scen.items():
        ...
        for tag, opts, exp in (("H", OPTS_H, exp_h), ("D", OPTS_D, exp_d)):
            ts = time.time()
            dec = model.decide(frames, opts, frames_per_state=fps, batch_size=1)[0]
            t = (time.time() - ts) * 1000
            ...
            print(f"PJ{fps} {'PASS' if got==exp else 'FAIL'} {name}{tag} exp={exp} got={got} "
                  f"conf={dec.confidence:.2f} mass={dec.allowed_mass:.2f} top={dec.top_token} "
